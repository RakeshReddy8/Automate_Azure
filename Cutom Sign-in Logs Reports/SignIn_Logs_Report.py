...
with open(f'SignIns_{only_date}_{current_date}.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
    ...
    for item in graph_result['value']:
        ...
        writer.writerow({
            ...
            'Multifactor authentication auth method': item['mfaDetail']['authMethod'] if item['mfaDetail'] is not None else '',
            'Multifactor authentication auth detail': item['mfaDetail']['authDetail'] if item['mfaDetail'] is not None else '',
